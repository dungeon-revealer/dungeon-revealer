define(["settings", "jquery", "brush"], function(settings, jquery, brush) {
  ...
  return function() {
    ...
    function create(parentElem, opts) {
      ...
      const imgUrl = opts.imgUrl || opts.mapImage;
      const callback = opts.callback || nop;
      const error = opts.error || nop;
    ...
    function getTouchCoordinates(e) {
      const viewportOffset = fowCanvas.getBoundingClientRect();
      const borderTop = parseInt($(fowCanvas).css("border-top-width"));
      const borderLeft = parseInt($(fowCanvas).css("border-left-width"));
    ...
    function getMouseCoordinates(e) {
      ...
      return {
        x:
          (e.clientX - viewportOffset.left - borderLeft) / getMapDisplayRatio(),
    ...
    function createImageCanvas(img) {
      ...
      const height = settings.maxHeight;
    ...
    function fitMapToWindow() {
      const oldWidth = parseInt(
        mapImageCanvas.style.width || mapImageCanvas.width,
        10
      );
      const oldHeight = parseInt(
        mapImageCanvas.style.height || mapImageCanvas.height,
        10
      );
      const newDims = getOptimalDimensions(
        oldWidth,
        oldHeight,
        $(window).width(),
        Infinity
    ...
    function findOptimalRhombus(pointCurrent, pointPrevious) {
      ...
      const pointCurrentCoordinates = constructCoordinates(pointCurrent);
      const pointPreviousCoordinates = constructCoordinates(pointPrevious);
    ...
    function setupCursorTracking() {
      ...
      function drawStartMousedown(e) {
        ...
        const cords = getMouseCoordinates(e);
        drawStart(cords);
      }
      ...
      function drawStartTouchstart(e) {
        ...
        isTouch = true;
        const cords = getTouchCoordinates(e);
        drawStart(cords);
      }
      ...
      function drawContinue(cords) {
        if (isDrawing) {
          const prevCords = points.length ? points[points.length - 1] : null;
          if (prevCords) {
      ...
      cursorCanvas.drawCursor = function(cords) {
        ...
        const cursorMask = constructMask(cords);
    ...
    function setUpDrawingEvents() {
      fowCanvas.drawInitial = function(coords) {
        ...
        const fowMask = constructMask(coords);
      ...
      fowCanvas.draw = function(points) {
        ...
        let pointCurrent = points[0]; //  the current point
        ...
        if (brushShape === "round") {
          ...
          for (let i = 1, len = points.length; i < len; i++) {
            ...
            const fowMask = constructMask(pointCurrent);
            fowContext.fillRect(
              fowMask.centerX,
              fowMask.centerY,
              fowMask.height,
            ...
            const optimalPoints = findOptimalRhombus(
              pointCurrent,
              pointPrevious
            );
            if (optimalPoints) {
      ...
      $("#btn-send").click(function() {
        createRender();
        const imageData = document.getElementById("render").src;
    ...
    function createPlayerMapImage(bottomCanvas, topCanvas) {
      const mergedCanvas = mergeCanvas(bottomCanvas, topCanvas);
      const mergedImage = convertCanvasToImage(mergedCanvas);
